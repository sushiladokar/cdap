...
public class CoreMessagingService extends AbstractIdleService implements MessagingService {
  ...
  @Inject
  public CoreMessagingService(CConfiguration cConf, TableFactory tableFactory) {
    this(cConf, tableFactory, TimeProvider.SYSTEM_TIME);
  }
  ...
  @Override
  public void publish(TopicId topicId, PublishRequest messages) throws TopicNotFoundException, IOException {
    try {
      messageTableWriterCache.get(topicId).persist(messages);
    } catch (ExecutionException e) {
      Throwable cause = Objects.firstNonNull(e.getCause(), e);
      Throwables.propagateIfPossible(cause, TopicNotFoundException.class, IOException.class);
  ...
  @Override
  public void storePayload(TopicId topicId, PublishRequest messages) throws TopicNotFoundException, IOException {
    try {
  ...
  @Override
  public void rollback(TopicId topicId, long transactionWritePointer) throws TopicNotFoundException, IOException {
    TopicMetadata metadata = getTopic(topicId);

    Exception failure = null;
    try (MessageTable messageTable = createMessageTable(metadata)) {
      messageTable.delete(topicId, transactionWritePointer);
    } catch (Exception e) {
      failure = e;
    }
    try (PayloadTable payloadTable = createPayloadTable(metadata)) {
      payloadTable.delete(topicId, transactionWritePointer);
    } catch (Exception e) {
      if (failure != null) {
    ...
    return CacheBuilder.newBuilder()
      ...
      .build(new CacheLoader<TopicId, ConcurrentMessageWriter>() {
        ...
        public ConcurrentMessageWriter load(TopicId topicId) throws Exception {
          TopicMetadata metadata = getTopic(topicId);
          PublishRequestWriter<?> messagesWriter = messageTable
            ? new MessageTablePublishRequestWriter(createMessageTable(metadata), timeProvider)
            : new PayloadTablePublishRequestWriter(createPayloadTable(metadata), timeProvider);
          return new ConcurrentMessageWriter(messagesWriter);
        }
      });
  }
