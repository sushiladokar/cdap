...
public class WorkflowClient {
  ...
  public void getWorkflowStatus(String namespaceId, String appId, String workflowId, String runId,
    ...
    // Construct request
    LOG.info("nsquare: From Workflow client");
    String url;
    if (Arrays.equals(Constants.Security.SSL_DISCOVERABLE_KEY.getBytes(), discoverable.getPayload())) {
      url = String.format("https://%s:%d/status", endpoint.getHostName(), endpoint.getPort());
    } else {
      url = String.format("http://%s:%d/status", endpoint.getHostName(), endpoint.getPort());
    }
